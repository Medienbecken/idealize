...
if ( ! class_exists( 'FEA_Form_Import' ) ) :
	...
	class FEA_Form_Import extends FEA_Admin_Tool {
		...
		function initialize() {
			...
			// vars
			$this->name  = 'import';
			$this->title = __( 'Import Admin Forms', FEA_NS );
		...
		function html() {
			...
			?>
		<p><?php _e( 'Select the JSON file you would like to import. When you click the import button below, your forms be imported.', FEA_NS ); ?></p>
		<div class="acf-fields">
			<?php

			acf_render_field_wrap(
				array(
					'label'    => __( 'Select File', FEA_NS ),
					'type'     => 'file',
		...
		<p class="acf-submit">
			<input type="submit" class="button button-primary" value="<?php _e( 'Import File', FEA_NS ); ?>" />
		</p>
			<?php
		...
		function submit() {
			...
			if ( empty( $_FILES['acf_import_file']['size'] ) ) {
				return acf_add_admin_notice( __( 'No file selected', FEA_NS ), 'warning' );
			}
			...
			// Check errors.
			if ( $file['error'] ) {
				return acf_add_admin_notice( __( 'Error uploading file. Please try again', FEA_NS ), 'warning' );
			}

			// Check file type.
			if ( pathinfo( $file['name'], PATHINFO_EXTENSION ) !== 'json' ) {
				return acf_add_admin_notice( __( 'Incorrect file type', FEA_NS ), 'warning' );
			}
			...
			if ( ! $json || ! is_array( $json ) ) {
				return acf_add_admin_notice( __( 'Import file empty', FEA_NS ), 'warning' );
			}
			...
			// Generate text.
			$text = sprintf( _n( 'Imported 1 form', 'Imported %s forms', $total, FEA_NS ), $total );
	...
	*/
			...
			if ( empty( $form['ID'] ) ) {
				$form['ID'] = wp_insert_post( array( 'post_type' => 'admin_form', 'post_title' => $form['id'] ) );
			...
			// Save form.
			$form = fea_instance()->form_builder->update_form_post( $form );
